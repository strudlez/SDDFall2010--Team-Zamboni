...
class main_window:
	...
	def add_hist_button(self,move_num, parent, color):
		...
		image = gtk.Image()
		if(color=="white"):
			pixbuf = gtk.gdk.pixbuf_new_from_file(zambogo.util.get_image("white.png"))
			scaled_buf = pixbuf.scale_simple(18,18,gtk.gdk.INTERP_BILINEAR)
			image.set_from_pixbuf(scaled_buf)
			
		else:
			pixbuf = gtk.gdk.pixbuf_new_from_file(zambogo.util.get_image("black.png"))
			scaled_buf = pixbuf.scale_simple(18,18,gtk.gdk.INTERP_BILINEAR)
			image.set_from_pixbuf(scaled_buf)
		image.show()
		label.show()
	...
	def __init__(self):
		...
		board = zambogo.engine.board.Board()
		self.goban = zambogo.actors.gobanactor.GobanActor(board,self)
		self.stage.add(self.goban)
		
		self.forfeit_b=gtk.Button("Resign")
		...
		self.window.show_all()
                ...
                self.library_window = zambogo.library.librarywindow.LibraryWindow()
